...
import org.ziglang.project.defaultZigPaths
...
class UtilsTest {
	...
	fun executeCommand() {
		...
		executeCommand(arrayOf(defaultZigPaths().first(), "version"), timeLimit = 5000L)
				.also(::println)
				.also(::assertNotNull)
	...
	fun versionOf() {
		if (!System.getenv("CI").isNullOrBlank()) return
		versionOf(defaultZigPaths().first()).let(::println)
	}
}
